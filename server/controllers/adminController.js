...
const {
  todayRevenueAndCommission,
} = require("../utils/todayRevenueAndCommission");
const { getIstDayRange } = require("../utils/getIstDayRange");
const ExcelJS = require("exceljs");
const { validateCryptoPayoutRequest } = require("../utils/cryptoValidator");
...
exports.getMyBalance = async (req, res) => {
  try {
    ...
    const unsettled = unsettledAgg[0] || {
      unsettledRevenue: 0,
      unsettledCount: 0,
    };
    ...
    const { totalTodayRevenue, totalPayinCommission, transactionCount } =
      await todayRevenueAndCommission(merchantObjectId);
    ...
    const todayPayoutCommissionAgg = await Payout.aggregate([
      ...
      },
      {
        $group: {
          _id: null,
          totalTodaysPayoutCommission: {
            $sum: { $ifNull: ["$commission", 0] },
          },
        },
      },
    ]);
    const totalTodaysPayoutCommission =
      todayPayoutCommissionAgg[0]?.totalTodaysPayoutCommission || 0;
    ...
    const weekTransactions = weekTransactionAgg[0] || {
      totalRevenue: 0,
      totalCommission: 0,
      totalRefunded: 0,
      transactionCount: 0,
    ...
    const weekPayouts = weekPayoutAgg[0] || {
      totalPaidOut: 0,
      totalPending: 0,
    ...
    const monthTransactions = monthTransactionAgg[0] || {
      totalRevenue: 0,
      totalCommission: 0,
      totalRefunded: 0,
      transactionCount: 0,
    ...
    const monthPayouts = monthPayoutAgg[0] || {
      totalPaidOut: 0,
      totalPending: 0,
      totalPayoutCommission: 0,
    };
    ...
    if (availableBalance > 0) {
      if (availableBalance > 1000) {
        ...
        maxPayoutGrossAmount = Math.min(availableBalance + 35.4, 1000);
      }
    }
    ...
    const nextSettlementStatus =
      nextUnsettledTransaction?.settlementStatus || null;
    ...
    res.json({
      ...
      balance: {
        ...
        totalTodaysPayoutCommission: parseFloat(
          totalTodaysPayoutCommission.toFixed(2)
        ),
        unsettled_revenue: unsettled.unsettledRevenue.toFixed(2),
        unsettled_commission: unsettledCommission.toFixed(2),
        unsettled_net_revenue: (
          unsettled.unsettledRevenue - unsettledCommission
        ).toFixed(2),
      ...
      balanceOfPastWeek: {
        ...
        total_commission: parseFloat(
          weekTransactions.totalCommission.toFixed(2)
        ),
        ...
        total_payout_commission: parseFloat(
          weekPayouts.totalPayoutCommission.toFixed(2)
        ),
        net_revenue: parseFloat(
          (
            weekTransactions.totalRevenue -
            weekTransactions.totalCommission -
            weekTransactions.totalRefunded
          ).toFixed(2)
        ),
      ...
      balanceOfPastMonth: {
        ...
        total_commission: parseFloat(
          monthTransactions.totalCommission.toFixed(2)
        ),
        ...
        total_payout_commission: parseFloat(
          monthPayouts.totalPayoutCommission.toFixed(2)
        ),
        net_revenue: parseFloat(
          (
            monthTransactions.totalRevenue -
            monthTransactions.totalCommission -
            monthTransactions.totalRefunded
          ).toFixed(2)
        ),
      ...
      settlement_info: {
        ...
        next_settlement_date:
          nextUnsettledTransaction?.expectedSettlementDate?.toISOString() ||
          null,
      ...
      transaction_summary: {
        ...
        avg_commission_per_transaction:
          settled.settledCount + unsettled.unsettledCount > 0
            ? (
                totalCommission /
                (settled.settledCount + unsettled.unsettledCount)
              ).toFixed(2)
            : "0.00",
      },
      payout_eligibility: {
        ...
        reason:
          availableBalance <= 0
            ...
            ? `Available balance is ₹${availableBalance.toFixed(
                2
              )} (after commission)`
            : "Eligible for payout",
      },
...
exports.searchTransactions = async (req, res) => {
  try {
    ...
    if (req.query.settlementStatus)
      query.settlementStatus = req.query.settlementStatus;
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to search transactions" });
  }
};
...
exports.searchPayouts = async (req, res) => {
  try {
    ...
    if (startDate || endDate) {
      ...
      dateOrConditions = [{ requestedAt: dateQuery }, { createdAt: dateQuery }];
    }
    ...
    if (beneficiaryName)
      query["beneficiaryDetails.accountHolderName"] = {
        $regex: beneficiaryName,
        $options: "i",
      };
    ...
    if (dateOrConditions.length > 0 && searchOrConditions.length > 0) {
      ...
      query.$and = [{ $or: dateOrConditions }, { $or: searchOrConditions }];
...
exports.getTransactionReport = async (req, res) => {
  try {
      ...
      worksheet
        .addRow({
          ...
          settlementDate: doc.settlementDate
            ? doc.settlementDate.toISOString()
            : "",
    ...
    return res
      .status(500)
      .json({ error: "Failed to generate report", detail: err.message });
  }
};
...
exports.getPayoutReport = async (req, res) => {
  try {
    ...
    if (q.description)
      query.description = { $regex: q.description, $options: "i" };
    if (q.beneficiaryName)
      query["beneficiaryDetails.accountHolderName"] = {
        $regex: q.beneficiaryName,
        $options: "i",
      };
    ...
    for await (const doc of cursor) {
      ...
      worksheet
        .addRow({
          ...
          transferMode:
            doc.transferMode === "bank_transfer" ? "Bank Transfer" : "UPI",
    ...
    return res
      .status(500)
      .json({ error: "Failed to generate payout report", detail: err.message });
  }
};
    ...
    for await (const doc of pCursor) {
      pSheet
        .addRow({
          ...
          status: doc.status || "",
          transferMode:
            doc.transferMode === "bank_transfer"
              ? "Bank Transfer"
              : doc.transferMode || "",
          beneficiaryName: doc.beneficiaryDetails?.accountHolderName || "",
          requestedAt: doc.requestedAt ? doc.requestedAt.toISOString() : "",
...
exports.requestPayout = async (req, res) => {
  try {
    ...
    console.log("beneficiaryDetails (full):", beneficiaryDetails);
    console.log(
      "beneficiaryDetails (partial):",
      beneficiaryDetails && {
        ...
        // Crypto fields
        cryptoWalletAddress: beneficiaryDetails.cryptoWalletAddress,
        cryptoNetwork: beneficiaryDetails.cryptoNetwork,
        cryptoCurrency: beneficiaryDetails.cryptoCurrency,
      }
    );

    if (!transferMode || !beneficiaryDetails) {
      console.log(
        "Validation failed: missing transferMode or beneficiaryDetails"
      );
      return res
        .status(400)
        .json({
    ...
    // ✅ Validate crypto payout request
    if (transferMode === "crypto") {
      const cryptoValidation = validateCryptoPayoutRequest(beneficiaryDetails);
      if (!cryptoValidation.valid) {
        console.log("Crypto validation failed:", cryptoValidation.errors);
        return res.status(400).json({
          success: false,
          error: "Crypto payout validation failed",
          errors: cryptoValidation.errors,
        });
      }
      console.log("✅ Crypto payout validation passed");
    }

    // --- Normalize and validate amount ---
    const parsedAmount =
      typeof rawAmount === "string" ? parseFloat(rawAmount) : rawAmount;
    const amountIsValidNumber =
    ...
    console.log(
      ...
      Array.isArray(settledTransactions) ? settledTransactions.length : 0
    );
    ...
    const totalSettledAmount = settledTransactions.reduce(
      (sum, t) => sum + t.amount,
      0
    );
    ...
    console.log(
      "Completed/reserved payouts count:",
      Array.isArray(completedPayouts) ? completedPayouts.length : 0
    );

    const totalReservedGross = completedPayouts.reduce(
      (sum, p) => sum + (p.grossAmount || p.amount || 0),
      0
    );
    console.log("Total reserved (gross) from payouts:", totalReservedGross);

    const availableBalance = parseFloat(
      (totalSettledAmount - totalReservedGross).toFixed(2)
    );
    ...
    console.log(
      `Payout requested: ₹${finalAmount} out of ₹${availableBalance} available`
    );
    ...
    try {
      ...
      return res
        .status(500)
        .json({ success: false, error: "Commission calculation failed" });
    }
    ...
    if (commissionType === "free") {
      console.log(
        "CommissionType is free -> attempting atomic decrement on merchant.freePayoutsUnder500"
      );
      ...
      console.log(
        "Result of findOneAndUpdate (decrement attempt):",
        updatedMerchant
          ? {
      ...
      if (!updatedMerchant) {
        console.log(
          "No updatedMerchant: free payout not available (concurrency or none left). Recomputing commission with free=0"
        );
        payoutCommissionInfo = calculatePayoutCommission(finalAmount, {
          ...merchant.toObject(),
          freePayoutsUnder500: 0,
        });
        ...
        console.log(
          "Atomic decrement succeeded. Updated merchant freePayoutsUnder500:",
          updatedMerchant.freePayoutsUnder500
        );
    ...
    if (netAmount <= 0) {
      ...
      return res
        ...
        .json({
          success: false,
          error: `Net payout must be positive. Computed net amount: ₹${netAmount}`,
        });
    }

    // Create payout with explicit fields
    const payoutId = `PAYOUT_REQ_${Date.now()}_${crypto
      .randomBytes(4)
      .toString("hex")}`;
    ...
    const remaining_balance = parseFloat(
      (availableBalance - grossAmount).toFixed(2)
    );

    console.log(
      `Payout saved: ${payoutId} gross ₹${grossAmount} net ₹${finalAmount} commission ₹${payoutCommission}`
    );
    ...
    return res
      .status(500)
      .json({ success: false, error: "Failed to create payout request" });
  }
};
...
exports.getMyPayouts = async (req, res) => {
  try {
    ...
    console.log(
      `📋 Admin ${req.user.name} fetching their payouts - Page ${pageNum}`
    );
    ...
    res.json({
      ...
      pagination: {
        ...
        limit: pageLimit,
        hasNextPage:
          pageNum < (pageLimit > 0 ? Math.ceil(totalCount / pageLimit) : 0),
      ...
      summary: {
        ...
        total_amount_requested: Number(agg.totalRequestedAmount || 0).toFixed(
          2
        ),
    ...
    console.log(
      `✅ Returned ${maskedPayouts.length} payouts to admin ${req.user.name}`
    );
  } catch (error) {
