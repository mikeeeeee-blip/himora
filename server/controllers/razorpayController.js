...
if (!process.env.RAZORPAY_KEY_ID || !process.env.RAZORPAY_KEY_SECRET) {
  ...
  if (!process.env.RAZORPAY_KEY_ID.startsWith("rzp_")) {
    console.warn(
      "⚠️ WARNING: RAZORPAY_KEY_ID should start with 'rzp_' (test) or 'rzp_live_' (live)"
    );
  }
  if (
    ...
    console.warn(
      "⚠️ WARNING: RAZORPAY_KEY_SECRET seems too short. Valid secrets are usually 32+ characters."
    );
  }
}
...
// Test Razorpay connection on startup (optional - can be removed if too verbose)
if (process.env.RAZORPAY_KEY_ID && process.env.RAZORPAY_KEY_SECRET) {
  // Don't test on startup, but log that we're ready
  console.log("🔧 Razorpay SDK initialized and ready");
}
...
exports.createRazorpayPaymentLink = async (req, res) => {
  try {
    ...
    const finalCallbackUrl =
      ...
      merchant.successUrl ||
      `${
        process.env.FRONTEND_URL || "https://payments.ninex-group.com"
      }/payment-success`;

    // Razorpay callback URL - points to our callback handler
    const razorpayCallbackUrl = `${
      process.env.BACKEND_URL || process.env.API_URL || "http://localhost:5000"
    }/api/razorpay/callback?transaction_id=${transactionId}`;
    ...
    console.log("🔍 Payment Link Options:", {
      ...
      description: paymentLinkOptions.description,
      customer: paymentLinkOptions.customer.name,
    });
    ...
    if (error.error) {
      ...
      errorMessage =
        error.error.description || error.error.message || errorMessage;
...
exports.createPhonePeDeepLink = async (req, res) => {
  try {
    ...
    if (parseFloat(amount) < 1)
      return res
        .status(400)
        .json({ success: false, error: "Amount must be at least ₹1" });
    ...
    const pa =
      ...
      (merchant && (merchant.upiId || merchant.vpa)) ||
      "";
    ...
    const paymentUrl =
      (paymentLink && (paymentLink.url || checkoutUrl)) ||
      (typeof paymentLink === "string" ? paymentLink : null);
    ...
    const transaction = new Transaction({
      ...
      netAmount:
        parseFloat(amount) - calculatePayinCommission(amount).commission,
      currency: "INR",
      description: description || `Payment for ${merchantName}`,
      status: "created",
...
const validateAuthHeader = (authHeader) => {
  ...
  if (timingSafeEqualStr(incoming.toLowerCase(), expected.toLowerCase()))
    return true;
  return false;
};

const validateSignatureHeader = (req) => {
  const phonepeSignature =
    req.headers["x-verify-signature"] ||
    req.headers["x-phonepe-signature"] ||
    req.headers["x-phonepe-sign"];
  ...
  if (timingSafeEqualStr(incoming.toLowerCase(), expectedHex.toLowerCase()))
    return { ok: true };
  ...
  try {
    ...
    if (authHeader) {
      ...
      if (!ok) {
        ...
        return res
          .status(403)
          .json({ success: false, error: "Invalid authorization" });
      }
      ...
      if (!sig || !sig.ok) {
        console.warn(
          "❌ Missing or invalid PhonePe signature header and no Authorization header"
        );
        return res.status(400).json({
          success: false,
    ...
    if (!payload || Object.keys(payload).length === 0) {
      ...
      return res
        .status(400)
        .json({ success: false, error: "Empty webhook payload" });
    }
  ...
  try {
    ...
    const update = {
      ...
      acquirerData: {
        ...
        bank_transaction_id:
          paymentDetail.instrument?.bank_transaction_id || null,
    ...
    if (!transaction) {
      console.warn(
        "⚠️ Transaction not found for transactionId:",
        transactionId
      );
      return;
    }
    ...
    console.log(
      `   - Expected settlement: ${expectedSettlement.toISOString()}`
    );
    ...
    const webhookPayload = {
      ...
      data: {
        ...
        expected_settlement_date:
          transaction.expectedSettlementDate.toISOString(),
    ...
    console.error(
      "❌ handlePhonePePaymentSuccess error:",
...
async function handlePhonePePaymentFailed(payload) {
  try {
    const transaction = await Transaction.findOne({
      phonepeReferenceId: payload.referenceId,
    }).populate("merchantId");
    if (!transaction) return;

    transaction.status = "failed";
    transaction.failureReason =
      payload.failureReason || payload.errorDescription;
    transaction.phonepePaymentId = payload.transactionId;
    transaction.updatedAt = new Date();

    await transaction.save();
    console.log(
      "❌ PhonePe Transaction marked as FAILED:",
  ...
  try {
    ...
    if (!transaction_id) {
      ...
      return res.redirect(
        `${
          process.env.FRONTEND_URL || "https://payments.ninex-group.com"
        }/payment-failed?error=missing_transaction_id`
      );
    }
          ...
          if (transaction.status !== "paid") {
            ...
            const update = {
              ...
              netAmount: parseFloat(
                (transaction.amount - commissionData.commission).toFixed(2)
              ),
            };
            ...
            if (
              updatedTransaction &&
              updatedTransaction.merchantId.webhookEnabled
            ) {
              const webhookPayload = {
                event: "payment.success",
                timestamp: new Date().toISOString(),
                ...
                data: {
                  ...
                  razorpay_payment_link_id:
                    updatedTransaction.razorpayPaymentLinkId,
                  ...
                  expected_settlement_date:
                    updatedTransaction.expectedSettlementDate.toISOString(),
    ...
    if (status === "success") {
      const redirectUrl =
        ...
        `${
          process.env.FRONTEND_URL || "https://payments.ninex-group.com"
        }/payment-success?transaction_id=${transaction_id}`;
    ...
    return res.redirect(
      `${
        process.env.FRONTEND_URL || "https://payments.ninex-group.com"
      }/payment-failed?error=callback_error`
    );
  }
};
...
exports.handleRazorpayWebhook = async (req, res) => {
  try {
    ...
    if (!webhookSecret) {
      console.error(
        "❌ RAZORPAY_WEBHOOK_SECRET not configured in environment variables"
      );
      console.error(
        "   - Please set RAZORPAY_WEBHOOK_SECRET in your .env file"
      );
      console.error(
        "   - You can find this in Razorpay Dashboard > Settings > Webhooks > Webhook Secret"
      );
      console.error(
        "   - Without the secret, webhooks cannot be verified and will be rejected"
      );
    }
    ...
    if (!signature) {
      ...
      console.warn(
        "     1. Webhook secret is not configured in Razorpay Dashboard"
      );
      ...
      if (isDevelopment && webhookSecret) {
        ...
        console.warn(
          "⚠️ WARNING: This is NOT secure and should NOT be used in production!"
        );
        ...
        try {
          ...
          if (payload && payload.event) {
            console.log(
              "📦 Processing webhook in development mode (no signature verification)"
            );
        ...
        return res.status(400).json({
          ...
          message:
            "Razorpay webhooks require X-Razorpay-Signature header for security. Please configure webhook secret in Razorpay Dashboard.",
        });
      }
    }
    ...
    if (!req.skipSignatureVerification) {
      ...
      if (!isValid) {
        ...
        console.warn(
          "   - Please verify RAZORPAY_WEBHOOK_SECRET matches the one in Razorpay Dashboard"
        );
        return res.status(401).json({
          success: false,
      ...
      console.warn(
        "⚠️ Skipping signature verification (development mode only)"
      );
    }
...
async function handleRazorpayPaymentSuccess(payload) {
  try {
    ...
    const paymentId =
      payload.payload?.payment?.entity?.id || payload.payment_id;
    ...
    if (!paymentLinkId && !paymentId && !referenceId) {
      console.warn(
        "⚠️ Missing payment link ID, payment ID, and reference ID in webhook payload"
      );
      return;
    }
    ...
    const paymentMethod =
      paymentEntity.method || paymentEntity.payment_method || "UPI";
    ...
    const update = {
      $set: {
        ...
        acquirerData: {
          ...
          bank_transaction_id:
            paymentEntity.acquirer_data?.bank_transaction_id || null,
        ...
        commission: commissionData.commission,
        netAmount: parseFloat(
          (amountPaid - commissionData.commission).toFixed(2)
        ),
      },
    };
    ...
    console.error(
      "❌ handleRazorpayPaymentSuccess error:",
      error.stack || error.message
    );
  }
}
...
async function handleRazorpayPaymentFailed(payload) {
  try {
    ...
    console.log(
      "❌ Razorpay Transaction marked as FAILED:",
