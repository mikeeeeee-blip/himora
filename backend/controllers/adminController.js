...
exports.searchTransactions = async (req, res) => {
  try {
    ...
    // ✅ SECURITY FIX: Always use authenticated merchant's ID from JWT token
    // Never trust merchantId from query params
    if (req.merchantId) {
      query.merchantId = new mongoose.Types.ObjectId(req.merchantId); // ✅ Added 'new' keyword
    } else {
      return res.status(403).json({
        success: false,
        error: 'Unauthorized: Merchant ID not found in token'
      });
    }
    ...
    console.log(`✅ Returned ${transactions.length} transactions for merchant ${req.merchantName}`);
...
exports.searchPayouts = async (req, res) => {
  try {
    ...
    if (beneficiaryName) {
      query['beneficiaryDetails.accountHolderName'] = { $regex: beneficiaryName, $options: 'i' };
    }
    ...
    if (search) {
      query.$or = [
        ...
        { 'beneficiaryDetails.upiId': { $regex: search, $options: 'i' } }
      ];
    }
    ...
    console.log(`✅ Returned ${payouts.length} payouts for merchant ${req.merchantName}`);
