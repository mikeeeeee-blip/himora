...
const QuickAnalytics = ({
  ...
  const processChartData = (data, type) => {
    ...
    data.forEach((item) => {
      ...
      if (type === "payin") {
        ...
        // Backend filters by createdAt, so use that for consistency
        // But prefer requestedAt for display if available (more accurate for user)
        const date = item.requestedAt || item.createdAt || item.created_at;
        if (!date) return;
        const dateObj = new Date(date);
        if (isNaN(dateObj.getTime())) return; // Invalid date
        dateKey = dateObj.toISOString().split("T")[0];
        // Use netAmount (amount after commission) for payouts
        amount = parseFloat(item.netAmount || item.amount || 0);
      } else {
        // settlement - use settlementDate if available, otherwise paidAt, fallback to createdAt
        const date =
          item.settlementDate ||
        ...
        if (isNaN(dateObj.getTime())) return; // Invalid date
        dateKey = dateObj.toISOString().split("T")[0];
        // Use netAmount (amount after commission) for settlements
        amount = parseFloat(
  ...
  const fetchChartData = async () => {
    try {
      ...
      // Set end date to end of day (23:59:59) to include full day
      // Backend parses this as: new Date(endDate), so we need full ISO string
      const endDate = new Date(now);
      endDate.setHours(23, 59, 59, 999);
      // Pass full ISO string so backend includes the full day
      ...
      try {
        ...
        console.log("✅ QuickAnalytics: Payin data fetched", {
          count: payinData.length,
          sample: payinData.slice(0, 2).map((t) => ({
            id: t.transactionId,
            amount: t.amount,
            date: t.createdAt,
            status: t.status,
      ...
      try {
        ...
        console.log("✅ QuickAnalytics: Payout data fetched", {
          count: payoutData.length,
          sample: payoutData.slice(0, 2).map((p) => ({
            id: p.payoutId,
      ...
      try {
        ...
        // Filter for only settled transactions
        settlementData = rawSettlements.filter((txn) => {
          if (!txn) return false;
          // Check settlement status (prefer camelCase, fallback to snake_case)
          const settlementStatus =
            txn.settlementStatus || txn.settlement_status;
          const isSettled = settlementStatus === "settled";

          // Also check if settlementDate exists (indicates it was settled)
          const hasSettlementDate = txn.settlementDate || txn.settlement_date;

          // Only include if explicitly settled (not just paid)
          return (
            isSettled || (hasSettlementDate && settlementStatus !== "unsettled")
          );
        });
        console.log("✅ QuickAnalytics: Settlement data fetched", {
          totalPaid: rawSettlements.length,
          settled: settlementData.length,
          startDate: startDateStr,
          endDate: endDateStr,
        });
      } catch (err) {
      ...
      // Fetch today's data for summary cards using balance endpoint
      // Endpoint: /api/payments/merchant/balance
      // Uses: paymentService.getBalance() → API_ENDPOINTS.BALANCE
      let todayPayin = 0;
      let todayPayoutCount = 0;
      let lastPayinAmount = 0;

      // Get today's date range for payout count
      ...
      try {
        // Get balance data which includes totalTodayRevenue
        // Endpoint: /api/payments/merchant/balance
        // Uses: paymentService.getBalance() → API_ENDPOINTS.BALANCE
        const balanceResult = await paymentService.getBalance();
        const balanceData = balanceResult.balance || {};

        // Get today's payin from balance endpoint (totalTodayRevenue)
        todayPayin = parseFloat(balanceData.totalTodayRevenue || 0);

        console.log("✅ QuickAnalytics: Balance data fetched", {
          totalTodayRevenue: todayPayin,
          balanceData: balanceData,
        });

        // Get last payin amount from chart data (most recent transaction)
        if (processedPayin.length > 0) {
        ...
        console.error("❌ QuickAnalytics: Balance fetch error", err.message);
        // Fallback: try to get today's payin from search if balance fails
        try {
          ...
          console.error(
            "❌ QuickAnalytics: Fallback payin fetch error",
            fallbackErr.message
          );
        }
      }
      ...
      try {
        ...
        console.log("✅ QuickAnalytics: Today payout count fetched", {
          count: todayPayoutCount,
        });
      } catch (err) {
        console.error(
