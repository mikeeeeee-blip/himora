...
const UpdateTransactionsPage = () => {
  ...
  useEffect(() => {
    fetchAllTransactionsByCreation();
  }, [statusFilter]);

  const fetchAllTransactionsByCreation = async () => {
    setLoading(true);
    setError('');
    setSuccess('');
    try {
      const filters = {
        limit: 1000 // Get all transactions (up to 1000)
      };
      if (statusFilter) {
        filters.status = statusFilter;
      }
      const response = await paymentService.getAllTransactionsByCreation(filters);
  ...
  const handleUpdate = async (transactionId) => {
    ...
    try {
      await paymentService.updateTransactionStatus(transactionId, newStatus);
      setSuccess(`Transaction ${transactionId} updated successfully!`);
      setEditingId(null);
      setNewStatus('');
      // Refresh the list
      await fetchAllTransactionsByCreation();
    } catch (e) {
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-gray-600">
                  Manage and update the status of all transactions (sorted by creation date)
                </p>
              </div>
              <button
                onClick={fetchAllTransactionsByCreation}
                disabled={loading}
