...
const SuperadminTransactionsPage = () => {
  ...
  const handleSettleTransaction = async (transactionId) => {
    if (
      !window.confirm(
        "Are you sure you want to manually settle this transaction?"
      )
    ) {
      return;
    }
    ...
    try {
      const result = await superadminPaymentService.settleTransaction(
        transactionId
      );
      setToast({
  ...
  const handlePreviewInvoice = (transactionId) => {
    setPreviewInvoiceId(transactionId);
  ...
  const handleDownloadInvoice = async (transactionId) => {
    ...
    try {
      await superadminPaymentService.downloadInvoice(transactionId);
      setToast({
  ...
  return (
    <div className="min-h-screen bg-[#001D22]">
            ...
            <div className="bg-[#122D32] border border-white/10 rounded-xl p-4 sm:p-6">
                  ...
                  <div className="flex gap-2 sm:gap-3 flex-wrap">
                    <button
                      ...
                      className={`flex items-center justify-center gap-2 bg-[#263F43] border border-white/10 hover:border-accent text-white px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm font-medium font-['Albert_Sans'] transition-all duration-200 hover:-translate-y-0.5 ${
                        showFilters ? "bg-accent/20 border-accent" : ""
                      }`}
                    >
                      <FiFilter />
                      <span>Filters</span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          handleFilterChange("settlementStatus", e.target.value)
                        }
                        className="w-full bg-[#001D22] border border-white/10 rounded-lg px-3 py-2 text-white text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent font-['Albert_Sans']"
                      >
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleFilterChange("merchantId", e.target.value)
                        }
              ...
              {loading ? (
                      ...
                      <tbody>
                        {transactions.map((txn) => (
                          ...
                          >
                            ...
                            <td
                              className="px-4 py-3 text-white/70 text-xs sm:text-sm font-['Albert_Sans'] hidden md:table-cell"
                              title={txn.orderId}
                            >
                            ...
                            <td
                              className="px-4 py-3 text-white/70 text-xs sm:text-sm font-['Albert_Sans'] hidden lg:table-cell"
                            ...
                            <td className="px-4 py-3 hidden lg:table-cell">
                              <div
                                className="text-white/70 text-xs sm:text-sm font-['Albert_Sans']"
                                title={txn.customerName}
                              >
                                {txn.customerName || "Anonymous"}
                              </div>
                              <div
                                className="text-white/50 text-xs font-['Albert_Sans']"
                                title={txn.customerEmail}
                              >
                            ...
                            <td className="px-4 py-3 hidden xl:table-cell">
                              {txn.settlementStatus?.toLowerCase() !==
                              "settled" ? (
                                <button
                                  onClick={() =>
                                ...
                                <span
                                  className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium font-['Albert_Sans'] bg-green-500/20 text-green-400`}
                                >
                            ...
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                <button
                                  onClick={() =>
                                    navigate(
                                      `/admin/transactions/${txn.transactionId}`
                                    )
                                  }
                                  className="flex items-center gap-1 px-2 py-1 bg-orange-500/20 hover:bg-accent/30 text-orange-500 rounded-lg text-xs font-medium font-['Albert_Sans'] transition-colors"
                                  title="View Details"
                                >
                                  <FiEye size={14} />
                                  <span className="hidden sm:inline text-orange-500">
                                    View
                                  </span>
                                </button>
                                {txn.status?.toLowerCase() === "paid" && (
                                  <>
                                    <button
                                      onClick={() =>
                                        handlePreviewInvoice(txn.transactionId)
                                      }
                                    ...
                                    <button
                                      onClick={() =>
                                        handleDownloadInvoice(txn.transactionId)
                                      }
                                      disabled={
                                        downloadingInvoiceId ===
                                        txn.transactionId
                                      }
                                      className="flex items-center gap-1 px-2 py-1 bg-green-500/20 hover:bg-green-500/30 text-green-400 rounded-lg text-xs font-medium font-['Albert_Sans'] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                ...
                <div className="flex flex-col items-center justify-center py-20 px-5">
                  ...
                  <p className="text-white/70 text-sm font-['Albert_Sans'] mb-4">
      ...
      <InvoicePreviewModal
        ...
        transactionId={previewInvoiceId}
        onClose={handleClosePreview}
        onDownload={() => {
          if (previewInvoiceId) {
            handleDownloadInvoice(previewInvoiceId);
            handleClosePreview();
          }
        }}
      />
